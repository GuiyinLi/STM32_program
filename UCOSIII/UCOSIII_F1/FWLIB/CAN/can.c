...
//Fpclk1的时钟在初始化的时候设置为36M,如果设置CAN_Mode_Init(CAN_SJW_1tq,CAN_BS2_8tq,CAN_BS1_9tq,4,CAN_Mode_Normal);
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	OS_ERR err;
	OSIntEnter();
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0)!= RESET)   //确认是否接收到信息
	{
		CAN_ClearITPendingBit(CAN1, CAN_IT_FMP0);
		CAN_Receive(CAN1, CAN_FIFO0, &RxMessage);
		OSSemPost(&CAN1_SEM,OS_OPT_POST_1,&err);//发送信号量 
	}
	OSIntExit();
}
#endif
