...
/************************************ 注意事项 ********************
      ...
      uint8 NRF24L01_TxPacket(uint8 *txbuf);//发送一个包的数据   发送数据位数已定TX_PLOAD_WIDTH
     调用格式：if(NRF24L01_TxPacket(tmp_buf)==TX_OK)   此为发送成功

     uint8 NRF24L01_RxPacket(uint8 *rxbuf);//接收一个包的数据    接收数据位数已定RX_PLOAD_WIDTH
     调用格式： if(NRF24L01_RxPacket(tmp_buf)==0)     此为接收到信息

     2  当没有接收端时，发送不可能成功，即NRF24L01_TxPacket(tmp_buf)！=TX_OK
...
void RX_Mode(void);//配置为接收模式
void TX_Mode(void);//配置为发送模式
uint8 NRF24L01_Write_Buf(uint8 reg, uint8 *pBuf, uint8 u8s);//写数据区
uint8 NRF24L01_Read_Buf(uint8 reg, uint8 *pBuf, uint8 u8s);//读数据区		  
