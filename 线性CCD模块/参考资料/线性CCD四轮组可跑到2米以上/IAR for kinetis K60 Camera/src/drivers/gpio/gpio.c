...
*                             野火嵌入式开发工作室
*
*  函数名称：gpio_ditalfilter
*  功能说明：设置对应口的digital filter
*  参数说明：PORTx       端口号（PORTA,PORTB,PORTC,PORTD,PORTE）
*            n           端口引脚
             div         busclock周期因子
*  函数返回：无
...
*  备    注：
*************************************************************************/
void gpio_digitalfilter(PORTx portx,u8 n,u8 div)          
{
         ASSERT( (n < 32u)  && (div < 32u) );           //使用断言检查输入、div
         PORT_DFER_REG(PORTX[portx])|= (1 << n);              //
         PORT_DFCR_REG(PORTX[portx]) = 0;                    //使用busclock
          if(div>=32)
           {
               div=32;                
            }                                   
         PORT_DFWR_REG(PORTX[portx])|=div;
