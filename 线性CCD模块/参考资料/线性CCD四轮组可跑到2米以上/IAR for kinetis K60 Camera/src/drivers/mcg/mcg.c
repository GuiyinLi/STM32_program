...
unsigned char pll_init(clk_option opt)
{
    ...
    MCG_C2 = 0;
#else
    // Enable external oscillator, RANGE=2 Frequency Range Select , HGO=1 High Gain Oscillator Select , EREFS=1 External Reference Select, LP=0 Low Power Select, IRCS=0
    MCG_C2 = MCG_C2_RANGE(2) | MCG_C2_HGO_MASK | MCG_C2_EREFS_MASK;
#endif

    //初始化晶振后释放锁定状态的振荡器和GPIO
    SIM_SCGC4 |= SIM_SCGC4_LLWU_MASK;  //LLWU Clock Gate Control
    LLWU_CS |= LLWU_CS_ACKISO_MASK;  //LLWU Control and Status Register
