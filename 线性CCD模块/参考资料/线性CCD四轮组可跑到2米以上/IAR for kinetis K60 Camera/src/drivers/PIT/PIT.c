...
void disable_pit(PITn pitn)
{
  DMA_IRQ_DIS(DMA_CH4);
  DMA_DIS(DMA_CH4);
  PIT_TCTRL(pitn) &=~( PIT_TCTRL_TEN_MASK | PIT_TCTRL_TIE_MASK );   // PITn定时器,并开PITn中断
  disable_irq(pitn + 68);			                                //开接收引脚的IRQ中断	                                //

 }

u32 now_time(PITn pitn)
{
  
  return PIT_CVAL(pitn);
}

void pit_init_M(PITn pitn, u32 cnt)
{
    //PIT 用的是 Bus Clock 总线频率
    //溢出计数 = 总线频率 * 时间
